...
Correctness release in the pure core. No API or output-shape change.

### Fixed

- **An unrecognised country name resolved as its own origin.** `normalizeOrigin` passed any 3+ character input straight through as the origin name, so a misspelling matched no Chapter 99 origin heading and the line resolved with only the universal layers — a silently understated duty stack. An origin is now accepted only as an ISO-3166 alpha-2 code, a canonical country name, or a listed alternative spelling (PRC, Viet Nam, USA, England, Myanmar, Czechia, Holland, UAE, and others); anything else returns the `ORIGIN_UNKNOWN` error.
- **Origin needles matched bare substrings of the heading text.** Oman matched "Romania" and "Romano", Niger matched "Nigeria", Dominica matched "Dominican Republic", and the United Kingdom's `uk` needle matched every "product of Ukraine" heading — layers attributed to the wrong country. Needles are now matched as whole words, and the names that contain another country's name as a whole word (Equatorial Guinea and Papua New Guinea against Guinea, the Democratic Republic of the Congo against the Republic of the Congo) are separated explicitly. On the bundled snapshot for 6109.10.00, candidate headings matched by origin name: OM 22 → 1, NE 3 → 0, DM 55 → 0, GN 5 → 2, GB 65 → 50; CN, VN, KR and CD unchanged.
- **Origin needles now cover the wording the schedule actually prints.** The United Kingdom no longer matches "product of Ukraine" headings, and Russia now matches the schedule's "the Russian Federation" wording (which restores headings 9903.82.14–.82.17 and 9903.90.08/.09, including a live +50% steel layer). Côte d'Ivoire is matched whichever apostrophe the heading prints — the schedule uses a backtick in one heading and a curly quote in another.
- **A cited range whose endpoints share no leading digit excluded everything.** When a range too wide to enumerate had no common prefix ("headings 0101.00.00 through 9503.00.00"), only the two endpoints stayed cited, so the chapter filter dropped every line inside the range the heading actually names. Such a range now covers everything, as the text says. No heading in the current snapshot cites one.
